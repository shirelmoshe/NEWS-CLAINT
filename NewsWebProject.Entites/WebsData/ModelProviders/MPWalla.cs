        ...
        public void GettingEachCategoryNews(List<TBRSSWebs> WebsData)
        {
            ...
            foreach (TBRSSWebs WebData in WebsData)
            {
                ...
                foreach (var item in items)
                {
                    ...
                    this.SubstringImageAndDescription(item.Element("description").Value, "<br/>", "</p>","src", out src, out description);

                    DataTable.Rows.Add(item.Element("title").Value, item.Element("enclosure").Attribute("url").Value, description, item.Element("link").Value, 0, WebData.RSSWebID,true,(DateTime)item.Element("pubDate"));
