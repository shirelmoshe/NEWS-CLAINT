        ...
        public void GettingEachCategoryNews(List<TBRSSWebs> WebsData)
        {
            ...
            foreach (TBRSSWebs WebData in WebsData)
            {
                ...
                foreach (var item in items)
                {
                    ...
                    DataTable.Rows.Add(item.Element("title").Value, item.Element(media + "content").Attribute("url").Value,item.Element("description").Value, item.Element("link").Value, 0, WebData.RSSWebID,true, (DateTime)item.Element("pubDate"));
